...
type FocusProxyElement = React.ComponentRef<typeof VisuallyHiddenPrimitive.VisuallyHiddenRoot>;
  ...
  (props: ScopedProps<NavigationMenuTriggerProps>, forwardedRef) => {
    ...
    return (
      <>
        ...
        {open && (
          <>
            <VisuallyHiddenPrimitive.VisuallyHiddenRoot
              aria-hidden
        ...
        <Primitive.a
          ...
          onClick={composeEventHandlers(
            ...
            event => {
              ...
              target.addEventListener(LINK_SELECT, e => onSelect?.(e), { once: true });
...
const NavigationMenuViewportImpl = React.forwardRef<NavigationMenuViewportImplElement, NavigationMenuViewportImplProps>(
  (props: ScopedProps<NavigationMenuViewportImplProps>, forwardedRef) => {
    ...
    /**
     * Update viewport size to match the active content node. We prefer offset dimensions over `getBoundingClientRect`
     * as the latter respects CSS transform. For example, if content animates in from `scale(0.5)` the dimensions would
     * be anything from `0.5` to `1` of the intended size.
     */
    ...
    return (
      <Primitive.div
        ...
        onPointerLeave={composeEventHandlers(props.onPointerLeave, whenMouse(context.onContentLeave))}
      >
        {Array.from(viewportContentContext.items).map(([value, { ref, forceMount, ...contentProps }]) => {
        ...
        <Primitive.button
          ...
          onKeyDown={composeEventHandlers(props.onKeyDown, event => {
            ...
            if (isFocusNavigationKey) {
              ...
              /**
               * Imperative focus during keydown is risky so we prevent React's batching updates to avoid potential
               * bugs. See: https://github.com/facebook/react/issues/20332
               */
