...
import { useComposedRefs } from '../../components/compose-refs';
...
// import type { DismissableLayerRoot } from '../dismissable-layer';
import { DismissableLayerBranch, DismissableLayerRoot } from '../dismissable-layer';
...
interface ToastViewportProps extends PrimitiveOrderedListProps {
   ...
   * landmarks. The available `{hotkey}` placeholder will be replaced for you.
   *
  ...
  (props: ScopedProps<ToastViewportProps>, forwardedRef) => {
    ...
    return (
      ...
      >
        ...
        {/**
         * tabindex on the the list so that it can be focused when items are removed. we focus the list instead of the viewport
         * so it announces number of items remaining.
         */}
...
const FocusProxy = React.forwardRef<FocusProxyElement, ScopedProps<FocusProxyProps>>((props, forwardedRef) => {
...
const Toast = React.forwardRef<ToastElement, ToastProps>((props: ScopedProps<ToastProps>, forwardedRef) => {
...
type DismissableLayerProps = React.ComponentPropsWithoutRef<typeof DismissableLayerRoot>;
...
const ToastImpl = React.forwardRef<ToastImplElement, ToastImplProps>(
  (props: ScopedProps<ToastImplProps>, forwardedRef) => {
    ...
    const composedRefs = useComposedRefs(forwardedRef, element => setNode(element));
    ...
    const startTimer = React.useCallback(
      (durationNumber: number) => {
        if (!durationNumber || durationNumber === Infinity) return;
        window.clearTimeout(closeTimerRef.current);
        closeTimerStartTimeRef.current = new Date().getTime();
        closeTimerRef.current = window.setTimeout(handleClose, durationNumber);
      },
      [handleClose]
    );
...
ToastImpl.displayName = TOAST_NAME;
...
const ToastAnnounceExclude = React.forwardRef<ToastAnnounceExcludeElement, ToastAnnounceExcludeProps>(
...
function handleAndDispatchCustomEvent<E extends CustomEvent, ReactEvent extends React.SyntheticEvent>(
  name: string,
...
const isDeltaInDirection = (delta: { x: number; y: number }, direction: SwipeDirection, threshold = 0) => {
