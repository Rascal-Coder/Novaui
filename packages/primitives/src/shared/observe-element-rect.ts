...
let rafId: number;
/**
 * Observes an element's rectangle on screen (getBoundingClientRect) This is useful to track elements on the screen and
 * attach other elements that might be in different layers, etc.
 */
function observeElementRect(
  ...
  return () => {
    const data = observedElements.get(elementToObserve);
    if (data === undefined) return;

    // start by removing the callback
    const index = data.callbacks.indexOf(callback);
    if (index > -1) {
