...
import { Presence, clamp, composeEventHandlers, useCallbackRef, useLayoutEffect } from '../../shared';
    ...
    return (
      <>
        ...
        <Primitive.div
          ...
          style={{
            /**
             * We don't support `visible` because the intention is to have at least one scrollbar if this component is
             * used and `visible` will behave like `auto` in that case
             * https://developer.mozilla.org/en-US/docs/Web/CSS/overflow#description
             *
             * We don't handle `auto` because the intention is for the native implementation to be hidden if using this
             * component. We just want to ensure the node is scrollable so could have used either `scroll` or `auto`
             * here. We picked `scroll` to prevent the browser from having to work out whether to render native
             * scrollbars or not, we tell it to with the intention of hiding them in CSS.
             */
        ...
        >
          {/**
           * `display: table` ensures our content div will match the size of its children in both horizontal and vertical axis so
           * we can determine if scroll width/height changed and recalculate thumb sizes. This doesn't account for children with
           * _percentage_ widths that change. We'll wait to see what use-cases consumers come up with there before trying to
           * resolve it.
           */}
          <div
...
const ScrollAreaScrollbarImpl = React.forwardRef<ScrollAreaScrollbarImplElement, ScrollAreaScrollbarImplProps>(
  (props: ScopedProps<ScrollAreaScrollbarImplProps>, forwardedRef) => {
    ...
    /**
     * We bind wheel event imperatively so we can switch off passive mode for document wheel event to allow it to be
     * prevented
     */
    React.useEffect(() => {
...
const ScrollAreaThumbImpl = React.forwardRef<ScrollAreaThumbImplElement, ScrollAreaThumbImplProps>(
  (props: ScopedProps<ScrollAreaThumbImplProps>, forwardedRef) => {
    ...
    React.useEffect(() => {
      ...
      if (viewport) {
        /**
         * We only bind to native scroll event so we know when scroll starts and ends. When scroll starts we start a
         * requestAnimationFrame loop that checks for changes to scroll position. That rAF loop triggers our thumb
         * position change when relevant to avoid scroll-linked effects. We cancel the loop when scroll ends.
...
const ScrollAreaCornerImpl = React.forwardRef<ScrollAreaCornerImplElement, ScrollAreaCornerImplProps>(
  (props: ScopedProps<ScrollAreaCornerImplProps>, forwardedRef) => {
    ...
    useResizeObserver(context.scrollbarX, () => {
      const scrollbarHeight = context.scrollbarX?.offsetHeight || 0;
      context.onCornerHeightChange(scrollbarHeight);
      setHeight(scrollbarHeight);
    });

    useResizeObserver(context.scrollbarY, () => {
      const scrollbarWidth = context.scrollbarY?.offsetWidth || 0;
      context.onCornerWidthChange(scrollbarWidth);
...
function useResizeObserver(element: HTMLElement | null, onResize: () => void) {
  ...
  useLayoutEffect(() => {
    ...
    if (element) {
       ...
       * notifications`. This means that ResizeObserver was not able to deliver all observations within a single
       * animation frame, so we use `requestAnimationFrame` to ensure we don't deliver unnecessary observations. Further
       * reading: https://github.com/WICG/resize-observer/issues/38
       */
