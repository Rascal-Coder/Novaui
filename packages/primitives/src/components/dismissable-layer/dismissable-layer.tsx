...
import { composeEventHandlers, useCallbackRef, useEscapeKeydown } from '../../shared';
...
const DismissableLayer = React.forwardRef<DismissableLayerElement, DismissableLayerProps>((props, forwardedRef) => {
   ...
   * `disableOutsidePointerEvents` would remove this layer from the stack and add it to the end again so the layering
   * order wouldn't be _creation order_. We only want them to be removed from context stacks when unmounted.
   */
  React.useEffect(() => {
    return () => {
      if (!node) return;
...
const DismissableLayerBranch = React.forwardRef<DismissableLayerBranchElement, DismissableLayerBranchProps>(
  (props, forwardedRef) => {
    const context = React.useContext(DismissableLayerContext);
...
/**
 * Listens for `pointerdown` outside a react subtree. We use `pointerdown` rather than `pointerup` to mimic layer
  ...
  React.useEffect(() => {
    const handlePointerDown = (event: PointerEvent) => {
      if (event.target && !isPointerInsideReactTreeRef.current) {
         ...
         * time the user stops touching the display and when the browser executres events. We need to ensure we don't
     ...
     * the document. This is not React specific, but rather how the DOM works, ie:
     *
...
/**
 * Listens for when focus happens outside a react subtree. Returns props to pass to the root (node) of the subtree we
 * want to check.
 */
