...
import { type Slot, createSlot } from '../../components/slot';
...
function createCollection<ItemElement extends HTMLElement, ItemData extends BaseItemData = BaseItemData>(name: string) {
  /* -----------------------------------------------------------------------------------------------
   * CollectionProvider
   * --------------------------------------------------------------------------------------------- */
  ...
  const [CollectionContextProvider, useCollectionContext] = createCollectionContext<ContextValue>(PROVIDER_NAME, {
    collectionElement: null,
    collectionRef: { current: null },
    collectionRefObject: { current: null },
    itemMap: new OrderedDict(),

    // 在老版本 JavaScript 中这是可能的
  ...
  const CollectionProviderImpl: React.FC<{
    ...
    const [collectionElement, setCollectionElement] = React.useState<CollectionElement | null>(null);
  ...
  const CollectionItemSlot = React.forwardRef<ItemElement, CollectionItemSlotProps>((props, forwardedRef) => {
    ...
    React.useEffect(() => {
      setItemMap(map => {
        ...
        if (!map.has(element)) {
          map.set(element, { ...(memoizedItemData as unknown as ItemData), element });
          return map.toSorted(sortByDocumentPosition);
        }

        return map
          .set(element, { ...(memoizedItemData as unknown as ItemData), element })
  ...
  return [{ Provider: CollectionProvider, Slot: CollectionSlot, ItemSlot: CollectionItemSlot }, functions] as const;
}
...
function sortByDocumentPosition<E extends HTMLElement, T extends BaseItemData>(
  ...
  if (!a[1].element || !b[1].element) return 0;
  return isElementPreceding(a[1].element, b[1].element) ? -1 : 1;
}
