...
const Select: React.FC<SelectProps> = (props: ScopedProps<SelectProps>) => {
  ...
  const isFormControl = trigger ? form || Boolean(trigger.closest('form')) : true;
...
const SelectContentImpl = React.forwardRef<SelectContentImplElement, SelectContentImplProps>(
  (props: ScopedProps<SelectContentImplProps>, forwardedRef) => {
    ...
    const [searchRef, handleTypeaheadSearch] = useTypeaheadSearch(search => {
      ...
      if (nextItem) {
        /**
         * Imperative focus during keydown is risky so we prevent React's batching updates to avoid potential bugs. See:
         * https://github.com/facebook/react/issues/20332
         */
              ...
              <SelectPosition
                ...
                onKeyDown={composeEventHandlers(contentProps.onKeyDown, event => {
                  ...
                  if (['ArrowUp', 'ArrowDown', 'Home', 'End'].includes(event.key)) {
                    ...
                    /**
                     * Imperative focus during keydown is risky so we prevent React's batching updates to avoid
                     * potential bugs. See: https://github.com/facebook/react/issues/20332
                     */
...
const SelectBubbleInput = React.forwardRef<HTMLSelectElement, SwitchBubbleInputProps>(
  ({ __scopeSelect, value, ...props }: ScopedProps<SwitchBubbleInputProps>, forwardedRef) => {
     ...
     *
     * We purposefully do not add the `value` attribute here to allow the value to be set programmatically and bubble to
     * any parent form `onChange` event. Adding the `value` will cause React to consider the programmatic dispatch a
     * duplicate and it will get swallowed.
     *
     * We use visually hidden styles rather than `display: "none"` because Safari autofill won't work otherwise.
     */
    return (
      <Primitive.select
        {...props}
...
/**
 * This is the "meat" of the typeahead matching logic. It takes in a list of items, the search and the current item, and
 ...
 * we only had that one character (ie. cycle through items starting with that character)
 *
 * We also reorder the items by wrapping the array around the current item. This is so we always look forward from the
 * current item, and picking the first item will always be the correct one.
 *
 * Finally, if the normalized search is exactly one character, we exclude the current item from the values because
 * otherwise it would be the first to match always and focus would never move. This is as opposed to the regular case,
 * where we don't want focus to move if the current item still matches.
 */
